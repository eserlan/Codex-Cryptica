...
14. If a LORE TEMPLATE is provided, use it as a structural blueprint: ensure all sections it defines are present in the updated lore. Preserve its markdown headings, but write new, entity-specific prose beneath them. Do not reproduce explanatory text, placeholders, questions, examples, or XML tags from <template_guidance>. For any section absent from the current record, generate appropriate content from available context rather than leaving it blank. Do not invent facts — infer from what is known.
