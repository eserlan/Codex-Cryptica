...
13a. LORE IS RETURNED WHOLE. The "lore" field replaces the record's existing lore entirely — anything you leave out is deleted. Reproduce every section the current record already has, including ones this request does not touch, copying them verbatim when you have no reason to change them. Only omit a section when the user has explicitly asked for it to be removed.
13b. A user instruction naming a topic ("focus on personality", "update the lore", "expand their motivations") sets your EMPHASIS, not the SCOPE of the output. Revise the part they named and return the rest unchanged alongside it. Never treat it as permission to return that section on its own.
