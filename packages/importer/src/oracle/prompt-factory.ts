...
4.  **Lore**: Detailed background information, history, or complex data — the "deep dive" content. Use the matching <template_guidance> block below only to understand each section's purpose. Preserve its markdown section headings, but write entity-specific prose beneath them. Do not reproduce explanatory text, placeholders, questions, examples, or XML tags from <template_guidance>. Include every section from that outline; when the source text doesn't cover a section, generate reasonable content by inferring from what's already known rather than leaving it blank or omitting the section — but do not invent major facts unsupported by the text.
