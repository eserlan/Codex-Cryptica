...
Before returning the adventure, run a consistency pass: ensure the voting rule and tally are mathematically correct for ${size} seats; every councillor's stance matches across all sections; each persuasion route directly addresses that councillor's stated motive rather than a generic bribe; the possible paths can actually achieve the required threshold; and the costly best solution genuinely resolves both sides of the central dilemma, not just the tally. Make any dependency where influencing one councillor changes another's options explicit. Fix any mismatch before responding.`;
}
