...
Before returning the adventure, run a consistency pass: ensure the voting rule and tally are mathematically correct for ${size} seats; every councillor's stance matches across all sections; each persuasion route directly addresses that councillor's stated motive rather than a generic bribe, and never targets a councillor whose vote is already secured; the possible paths can actually achieve the required threshold given the vote changes they describe; and the costly best solution genuinely resolves both sides of the central dilemma AND improves on or replaces the original proposal, not just the tally. Make any dependency where influencing one councillor changes another's options explicit. Fix any mismatch before responding.`;
}
