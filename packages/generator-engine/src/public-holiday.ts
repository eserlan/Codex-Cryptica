...
Return JSON with title, summary, content, lore, labels, status, and observances. Emit the keys in that order. title, summary, content and lore must each be a single Markdown string (never a nested object or array), so the text can be shown as it is written. The content is the table-facing calendar entry or overview. For each observance include name, type, when, observers, origin, traditions, foodOrDress, taboos, variations, outsiderMisunderstanding, publicBelief, optional hiddenTruth, and tension. Use distinct concepts; dates should be distributed plausibly, with religious, civic, seasonal, memorial and cultural variety where appropriate. Reuse motifs across the set when it makes sense, allow competing groups to reinterpret an event, and do not give every historical event its own holiday. A hook is an invitation, not a required plot.

Keep the summary to one or two plain sentences. Write lore as scannable GM reference, not prose: a one-sentence overview, then short Markdown headings such as Society, Land and livelihood, Calendar logic and Secrets, each with two to four brief bullets of at most one sentence. Do not write paragraphs longer than three sentences, and do not repeat what the content already says.
