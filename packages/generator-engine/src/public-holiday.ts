...
Use cultural causality rather than novelty for its own sake. Each day must answer why people care, how geography, climate, religion, history, or politics shaped it, what behaviour it produces, who benefits, who rejects it, and how its meaning has changed. Supplied people and events are candidates, not a checklist: select only plausible commemorations and do not force every input into the result. Avoid Gregorian dates unless the culture uses that calendar; give seasonal, relative, lunar, or astronomical timing instead.
...
For a calendar, begin content with a compact overview giving the mix, then expand all ${resolved.count} observances. The observances array must contain exactly ${resolved.count} matching entries in the same order as the expanded content. For one observance, provide exactly one entry.
