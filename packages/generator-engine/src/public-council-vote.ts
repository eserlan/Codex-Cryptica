...
${NAME_BAN_PROMPT}
${sessionContext}
Before returning, run a consistency pass: the voting rule and threshold in "Voting Procedure" must be mathematically achievable with ${resolved.councilSize} seats and must match the tally implied by "Current Vote Estimate"; every councillor's stance is identical everywhere they appear (Council Members, Current Vote Estimate, Possible Paths); each listed persuasion method genuinely addresses that specific councillor's stated agenda rather than a generic bribe; every path in "Possible Paths", including the costly best solution, can actually reach the required threshold given the members' real stances; the costly best solution must resolve both sides of the central dilemma — securing the vote AND addressing whatever made the proposal contested in the first place — not just win the tally; and where swaying one councillor changes what would persuade another, state that dependency explicitly in their bullets. Fix any mismatch before responding.
