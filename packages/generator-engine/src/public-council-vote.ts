...
${NAME_BAN_PROMPT}
${sessionContext}
Treat each councillor's initial stance as fixed source data: every persuasion route or coalition in "Possible Paths" must explain exactly how a specific councillor's vote changes from that stated initial stance, and the final tally in any resolved path must be recalculated from those changes under the stated "Voting Procedure" — do not describe the party spending effort or resources on councillors whose vote is already secured (already supporting or already opposed and unmovable). The costly best solution must improve on or replace the original proposal in some concrete way, not merely secure the same original proposal through a more ethical or less costly route.
Before returning, run a consistency pass: the voting rule and threshold in "Voting Procedure" must be mathematically achievable with ${resolved.councilSize} seats and must match the tally implied by "Current Vote Estimate"; every councillor's stance is identical everywhere they appear (Council Members, Current Vote Estimate, Possible Paths); each listed persuasion method genuinely addresses that specific councillor's stated agenda rather than a generic bribe, and is never aimed at a councillor whose vote is already secured; every coalition path in "Possible Paths", including the costly best solution, obeys the stated "Voting Procedure" and can actually reach the required threshold given the members' real stances and the vote changes it describes; each changed vote in a path must directly resolve, reward, or override that councillor's true agenda (not just their public position); the costly best solution must resolve both sides of the central dilemma AND improve on or replace the original proposal — not just win the same proposal's tally; and where swaying one councillor changes what would persuade another, state that dependency explicitly in their bullets. Fix any mismatch before responding.
