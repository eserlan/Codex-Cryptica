...
describe("Temporal Validation", () => {
  describe("validateTemporal", () => {
    ...
    it("returns an error if year is missing", () => {
      ...
      expect(validateTemporal({})).toBe(
        "Year is required for chronological placement.",
      );
      // @ts-expect-error Testing invalid input
      expect(validateTemporal({ year: undefined })).toBe(
        "Year is required for chronological placement.",
      );
      // @ts-expect-error Testing invalid input
      expect(validateTemporal({ year: null })).toBe(
        "Year is required for chronological placement.",
      );
    });
    ...
    it("returns an error if year is not a valid number", () => {
      // @ts-expect-error Testing invalid input
      expect(validateTemporal({ year: "2023" })).toBe(
        "Year must be a valid number.",
      );
      expect(validateTemporal({ year: NaN })).toBe(
        "Year must be a valid number.",
      );
    });

    it("returns an error if month is out of bounds", () => {
      expect(validateTemporal({ year: 2023, month: 0 })).toBe(
        "Month must be between 1 and 12.",
      );
      expect(validateTemporal({ year: 2023, month: 13 })).toBe(
        "Month must be between 1 and 12.",
      );
    });

    it("returns an error if day is out of bounds", () => {
      expect(validateTemporal({ year: 2023, month: 5, day: 0 })).toBe(
        "Day must be between 1 and 31.",
      );
      expect(validateTemporal({ year: 2023, month: 5, day: 32 })).toBe(
        "Day must be between 1 and 31.",
      );
    });
  });
  ...
  describe("validateTemporalRange", () => {
    ...
    it("returns an error if start date is invalid", () => {
      // @ts-expect-error Testing invalid input
      expect(validateTemporalRange({ year: undefined }, { year: 2023 })).toBe(
        "Start date error: Year is required for chronological placement.",
      );
    });

    it("returns an error if end date is invalid", () => {
      // @ts-expect-error Testing invalid input
      expect(validateTemporalRange({ year: 2023 }, { year: undefined })).toBe(
        "End date error: Year is required for chronological placement.",
      );
    });
    ...
    it("returns an error if start year is after end year", () => {
      expect(validateTemporalRange({ year: 2024 }, { year: 2023 })).toBe(
    ...
    it("returns an error if start month is after end month in the same year", () => {
      ...
      ).toBe("Start month cannot be after end month.");
    });
