...
export function parseOracleResponse(text: string): OracleParseResult {
  ...
  const typeMarkerRegex =
    /^(?:#+\s*|\*\*|__)?(?:Type|Category)\b(?:\*\*|__|:|\s)*:?\s*/im;
  const imageMarkerRegex =
    /^(?:#+\s*|\*\*|__)?(?:ImageURL|ImageUrl|Image)\b(?:\*\*|__|:|\s)*:?\s*/im;
  const thumbMarkerRegex =
    /^(?:#+\s*|\*\*|__)?(?:Thumbnail|Thumb)\b(?:\*\*|__|:|\s)*:?\s*/im;
  const connMarkerRegex =
    /^(?:#+\s*|\*\*|__)?(?:Connections|Related|Links)\b(?:\*\*|__|:|\s)*:?\s*/im;
  ...
  if (hasChronicle || hasLore) {
    ...
    const wasSplit =
      (chronicle !== "" && lore !== "") || hasChronicle || hasLore;
...
function normalizeType(raw?: string): string | undefined {
  ...
  if (t.includes("person") || t.includes("npc") || t.includes("character"))
    return "character";
  if (t.includes("place") || t.includes("location") || t.includes("settlement"))
    return "location";
  if (
    t.includes("faction") ||
    t.includes("guild") ||
