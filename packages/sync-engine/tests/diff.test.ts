...
describe("DiffAlgorithm", () => {
  ...
  describe("Pull Direction (Folder -> OPFS)", () => {
    ...
    it("should skip new file in OPFS", async () => {
    ...
    it("should handle initial match (no registry) - different size -> import (pull wins)", async () => {
    ...
    it("should skip local (FS) deletion (don't push to FS)", async () => {
      const opfs = createMeta(1000);
    ...
    it("should skip OPFS deletion (don't delete in FS)", async () => {
      const fs = createMeta(1000);
      const registry = createEntry(1000, 100, "abc");
      const result = await DiffAlgorithm.calculateAction(
        path,
  ...
  describe("Push Direction (OPFS -> Folder)", () => {
    it("should skip new file in local (FS)", async () => {
      const fs = createMeta(1000);
      const result = await DiffAlgorithm.calculateAction(
        path,
        fs,
    ...
    it("should handle conflict - OPFS wins", async () => {
