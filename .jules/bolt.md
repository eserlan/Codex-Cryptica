...
## 2024-07-11 - [Performance Insight: Refactoring Object.values(vault.entities) to pre-cached vault.allEntities]
**Learning:** Re-affirming that replacing `Object.values(vault.entities)` with the pre-cached `vault.allEntities` array inside `$derived` blocks successfully reduces intermediate array allocations, memory pressure, and garbage collection overhead during reactive updates in UI components like `EntityProposals.svelte`.
**Action:** Always prefer using `vault.allEntities` over `Object.values(vault.entities)` when iterating over entities in reactive Svelte contexts. Be sure to update corresponding unit tests to mock `allEntities` appropriately to prevent test crashes.
