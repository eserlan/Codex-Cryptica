...
test("proxy call overhead is under 200ms", async ({ page }) => {
  ...
  page.on("response", async (response) => {
    ...
    if (url.includes("oracle-proxy.espen-erlandsen.workers.dev")) {
  ...
  // Log for debugging
  console.log(
    `Proxy performance - Avg: ${avgTime.toFixed(2)}ms, Max: ${maxTime.toFixed(2)}ms, Samples: ${responseTimes.length}`,
  );
});
