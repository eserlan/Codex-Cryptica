...
test.describe('Vault Management', () => {
  ...
  test('should rename and delete a vault', async ({ page }) => {
    ...
    const defaultRow = modal.getByRole('button', { name: /Default Vault|Local Vault/ }).first();
    ...
    const input = modal.locator('input').filter({ hasNot: modal.getByPlaceholder('Vault Name...') }).first();
    ...
    await expect(modal.locator('.group', { hasText: newName })).not.toBeVisible();
  });
});
