...
{#if manager.session && manager.recap}
  ...
  {@const hasDetails =
    recap.objectives.length ||
    recap.activeCharacters.length ||
    recap.knownFacts.length ||
    recap.recentTurnSummaries.length}
  <section
    class="rounded-lg border border-theme-border p-3"
    aria-labelledby="adventure-state-heading"
  >
    <h3
      id="adventure-state-heading"
      class="text-xs font-semibold uppercase tracking-wide text-theme-secondary"
    >
      Current situation
    </h3>
    {#if recap.location}<p class="mt-1 text-sm text-theme-primary">
        <strong>{recap.location.text}</strong>
      </p>{/if}
    {#if recap.situation}<p class="text-sm text-theme-primary">
    ...
    {#if hasDetails}
      <details class="mt-2 text-sm">
        ...
        {#if recap.knownFacts.length}<h4
          ...
          <ul class="list-disc pl-5 text-sm text-theme-secondary">
            {#each recap.knownFacts as fact (fact.id)}<li>
                {fact.text}
              </li>{/each}
          </ul>{/if}
        {#if recap.recentTurnSummaries.length}<h4
            class="mt-3 text-sm font-medium text-theme-primary"
          >
            Recap: what just happened
          </h4>
          <ol class="mt-1 list-decimal space-y-1 pl-5 text-theme-secondary">
