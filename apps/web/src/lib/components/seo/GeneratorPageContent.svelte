<script lang="ts">
  ...
  let handoffState = $state<GeneratorHandoffState>(() => {
    const initialized = initializeHandoffState(_initialSlug);
    return {
      ...initialized,
  ...
  setupHandoffNavigation(slug, handoffState, (updates) => {
    Object.assign(handoffState, updates);
    if (updates.questPremise) plotTwist.premise = updates.questPremise;
    if (updates.factionContext)
      factionRoster.factionContext = updates.factionContext;
    if (updates.npcContext) npc.campaignContext = updates.npcContext;
  });
