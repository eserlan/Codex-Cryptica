<script lang="ts">
    ...
    let { entity, isEditing, editTitle = $bindable(), onClose } = $props<{
        entity: Entity;
        isEditing: boolean;
        editTitle: string;
        onClose: () => void;
...
{#if isObscured}
    ...
    >
        ...
        <h3 class="text-xl font-bold text-theme-text uppercase tracking-widest mb-2">
            Protocol Redacted
        </h3>
        <p class="text-xs text-theme-muted leading-relaxed max-w-xs">
            This archive entry is currently obscured by Fog of War. Switch to
            Admin Mode or remove the <code class="text-amber-500">hidden</code> tag
            to decrypt.
        </p>
        <button
            onclick={onClose}
...
<div class="p-4 md:p-6 border-b border-theme-border bg-theme-surface">
    <div class="flex justify-between items-start mb-2">
        ...
        {:else}
            <h2 class="text-2xl md:text-3xl font-bold text-theme-text font-serif tracking-wide">
                {entity.title}
            </h2>
        {/if}
    ...
    <div class="mt-4 mb-2 space-y-2">
        <div class="flex flex-wrap gap-1.5 min-h-[24px]">
            ...
            {#if !entity.labels?.length && vault.isGuest}
                <span class="text-[9px] text-theme-muted italic uppercase tracking-tighter">
                    No labels
                </span>
            {/if}
        </div>
