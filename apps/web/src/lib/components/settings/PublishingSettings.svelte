<script lang="ts">
  ...
  const activeRegistry = $derived(
    activeVaultId ? publishingService.publishedVaults[activeVaultId] : undefined
  );
...
<div class="space-y-6">
  <div class="bg-theme-bg/30 border border-theme-border/50 p-6 rounded-lg space-y-4">
    <div class="flex items-start gap-4">
      <span class="icon-[lucide--share-2] w-8 h-8 text-theme-primary shrink-0 mt-1"></span>
      <div>
        <h3 class="text-md font-bold uppercase tracking-wider text-theme-primary font-header">
          Player-Safe Snapshot Hosting
        </h3>
        <p class="text-sm text-theme-text/80 leading-relaxed mt-2">
          Publishing compiles a read-only snapshot of your campaign, cleans all GM secrets (lore and art direction fields), redacts private links, and uploads it to Cloudflare R2. This allows your players to browse your world asynchronously and offline without exposing your active workspace or secrets.
        </p>
      </div>
    </div>
  </div>
  ...
  {#if publishingService.isPublishing}
    ...
    <div class="bg-theme-bg/50 border border-theme-border p-6 rounded-lg space-y-4 animate-pulse">
      <div class="flex justify-between items-center text-sm">
        <span class="font-header uppercase tracking-wider text-theme-primary font-bold">
      ...
      <div class="w-full bg-theme-border/30 h-2 rounded-full overflow-hidden">
        <div class="bg-theme-primary h-full transition-all duration-300" style="width: {publishingService.progress}%"></div>
      </div>
      <p class="text-xs text-theme-text/60">
        You can close this settings window. The upload runs safely in the background.
      </p>
    </div>
  {:else if activeRegistry}
    <!-- Already Published Status -->
    <div class="border border-theme-border rounded-lg overflow-hidden">
      <div class="bg-theme-primary/5 px-6 py-4 border-b border-theme-border flex justify-between items-center">
        <div>
          <span class="text-xs text-theme-primary font-header uppercase tracking-widest font-bold">Status</span>
          <h4 class="text-lg font-bold text-theme-text uppercase font-header mt-1">Active Publication</h4>
        </div>
        <div class="flex items-center gap-1.5 px-3 py-1 bg-green-500/10 border border-green-500/20 text-green-400 text-xs rounded font-bold uppercase font-header tracking-wider">
      ...
      <div class="p-6 space-y-6">
        <div class="space-y-2">
          <label for="shareable-link-input" class="text-xs text-theme-text/60 font-header uppercase tracking-wider block">Shareable Link</label>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div class="p-3 bg-theme-surface border border-theme-border/40 rounded">
            <span class="text-[10px] text-theme-text/50 font-header uppercase tracking-wider block">Entities</span>
            <span class="text-lg font-bold font-header text-theme-text mt-1 block">{activeRegistry.stats.entityCount}</span>
          </div>
          <div class="p-3 bg-theme-surface border border-theme-border/40 rounded">
            <span class="text-[10px] text-theme-text/50 font-header uppercase tracking-wider block">Connections</span>
            <span class="text-lg font-bold font-header text-theme-text mt-1 block">{activeRegistry.stats.relationshipCount}</span>
          </div>
          <div class="p-3 bg-theme-surface border border-theme-border/40 rounded">
            <span class="text-[10px] text-theme-text/50 font-header uppercase tracking-wider block">Uploaded Assets</span>
            <span class="text-lg font-bold font-header text-theme-text mt-1 block">{activeRegistry.stats.assetCount}</span>
          </div>
          <div class="p-3 bg-theme-surface border border-theme-border/40 rounded">
            <span class="text-[10px] text-theme-text/50 font-header uppercase tracking-wider block">Last Published</span>
            <span class="text-xs font-bold font-header text-theme-text mt-1 block">
              {new Date(activeRegistry.publishedAt).toLocaleDateString()}
            </span>
          </div>
        </div>

        <div class="flex flex-col sm:flex-row justify-end gap-3 border-t border-theme-border pt-6">
          <button
  ...
  {:else}
    <!-- Not Published Yet -->
    <div class="border border-theme-border rounded-lg p-8 text-center space-y-5 bg-theme-surface/30">
      <div class="mx-auto w-12 h-12 rounded-full bg-theme-primary/10 flex items-center justify-center">
        <span class="icon-[lucide--share-2] w-6 h-6 text-theme-primary"></span>
      </div>
      <div class="max-w-md mx-auto space-y-2">
        <h4 class="text-md font-bold uppercase font-header tracking-wider text-theme-text">Not Yet Shared</h4>
        <p class="text-sm text-theme-text/60 leading-relaxed">
          Generate your first read-only snapshot link to share your campaign lore with players.
        </p>
      </div>
      <button
        onclick={() => (showPreviewModal = true)}
