<script lang="ts">
  ...
  let isConfigured = $derived(
    adapter?.isConfigured() ||
      (typeof window !== "undefined" && (window as any).TEST_FORCE_CONFIGURED),
  );
</script>
        ...
        {:else if !isConnected}
          <div class="flex flex-col gap-3">
            <p class="text-theme-muted italic">
              Mirror your lore to Google Drive for multi-device access. This is
              entirely opt-in.
            </p>
            <button
          ...
          <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between">
              <div class="flex flex-col">
                <span class="text-xs text-theme-muted uppercase">Account</span>
                <span
            ...
            >
              <div class="flex justify-between mb-1">
                ...
                <span
                  class={status === "ERROR"
                    ? "text-theme-accent"
                    : "text-theme-primary font-bold"}>{status}</span
                >
              </div>
              ...
              {#if isSyncing && $syncStats.stats?.phase}
                <div class="mb-3">
                  ...
                  >
                    ...
                    <span
                      >{$syncStats.stats.current} / {$syncStats.stats
                        .total}</span
                    >
                  </div>
                  <div
                  ...
                  >
                    <div
                      ...
                      style="width: {$syncStats.stats.total &&
                      $syncStats.stats.total > 0
                        ? (($syncStats.stats.current || 0) /
              ...
              {#if $syncStats.stats}
                ...
                >
                  <div class="text-theme-muted uppercase">Uploaded</div>
                  <div class="text-theme-text/80">
                    {$syncStats.stats.filesUploaded}
                  </div>
                  <div class="text-theme-muted uppercase">Downloaded</div>
                  <div class="text-theme-text/80">
            ...
            <div class="flex gap-2">
              <button
                ...
                disabled={isSyncing || !$cloudConfig.enabled || isLoading}
              >
                {#if isSyncing || isLoading}
                  ...
                  ></span>
                  {isSyncing
                    ? `${themeStore.jargon.syncing.toUpperCase()}...`
                    : "CONNECTING..."}
                {:else if !hasToken}
                  RECONNECT
                {:else}
