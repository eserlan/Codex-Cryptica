...
<div class="p-4 border border-theme-border rounded-lg bg-theme-bg/30 mt-4">
  <div class="flex items-center justify-between mb-2">
    <div class="flex items-center gap-2">
      <div
        class="w-2 h-2 bg-theme-primary rounded-full animate-pulse"
      ></div>
      <h3
  ...
  <p class="text-sm text-theme-text/80 mb-4 leading-relaxed">
    The Oracle uses Google Gemini for AI-powered lore assistance. 
    Use the system proxy for free access, or add your own API key for direct access.
  </p>
  ...
  <div class="mb-6">
    ...
    {#if oracle.apiKey}
      <div class="p-4 bg-theme-primary/5 border border-theme-primary/20 rounded flex items-center gap-3">
        <span class="text-theme-primary icon-[heroicons--sparkles] w-5 h-5"></span>
        <div class="flex-1">
          <span class="text-base text-theme-text font-bold uppercase font-header tracking-wider">
        ...
        <span class="text-xs text-theme-muted font-mono">••••{oracle.apiKey?.slice(-4)}</span>
      </div>
    {:else}
      <div class="p-4 bg-theme-accent/5 border border-theme-accent/20 rounded flex items-center gap-3">
