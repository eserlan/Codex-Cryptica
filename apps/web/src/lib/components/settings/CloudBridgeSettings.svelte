...
<div class="p-4 border rounded-lg bg-base-200">
    ...
    <div class="flex gap-2 items-center">
      ...
      {#if $cloudConfig.enabled}
        <button
          ...
          on:click={() =>
            import("$lib/cloud-bridge/worker-bridge").then((m) =>
              m.workerBridge.startSync(),
            )}
        >
          Sync Now
        </button>
  ...
  {#if $cloudConfig.enabled && $cloudConfig.connectedEmail}
    <div
      class="mt-4 p-2 bg-success/10 text-success rounded text-sm flex items-center gap-2 justify-between"
    >
      ...
      {#if $syncStats.stats}
        <span class="text-xs opacity-70">
          Uploaded: {$syncStats.stats.filesUploaded} | Downloaded: {$syncStats
