<script lang="ts">
  ...
  const getEraLabel = (year: number) => {
    const era = graph.eras.find(
      (e) =>
        year >= e.start_year &&
        (e.end_year === undefined || year <= e.end_year),
    );
          ...
          {#each entries as entry, i}
            <div
              class="relative flex items-center {i % 2 === 0
                ? 'md:flex-row'
                : 'md:flex-row-reverse'} gap-8"
            >
              <!-- Connector Dot -->
              <div
                class="absolute left-4 md:left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-green-500 z-10 border-4 border-black box-content"
              ></div>

              <!-- Content (Alternate sides on desktop) -->
              <div
                class="pl-12 md:pl-0 md:w-1/2 flex {i % 2 === 0
                  ? 'md:justify-end'
                  : 'md:justify-start'}"
              >
