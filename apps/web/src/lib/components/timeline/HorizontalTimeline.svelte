  ...
  {#each timelineStore.filteredEntries as entry, i}
    <div class="relative flex flex-col items-center min-w-[280px] group">
      ...
      {#if i < timelineStore.filteredEntries.length - 1}
        <div class="absolute top-[50%] left-1/2 w-[calc(100%+48px)] h-px bg-gradient-to-r from-green-900/50 via-green-500/20 to-green-900/50 z-0"></div>
      {/if}

      <!-- Node Marker -->
      <div class="w-4 h-4 rounded-full bg-green-500 shadow-[0_0_15px_rgba(34,197,94,0.5)] z-10 mb-8 group-hover:scale-125 transition-transform"></div>
