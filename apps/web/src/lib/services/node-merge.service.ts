import type { INodeContent, IMergedContentProposal } from '../../../../../packages/editor-core/src/operations/merge-utils';
import { mergeFrontmatter, concatenateBody } from '../../../../../packages/editor-core/src/operations/merge-utils';
...
export class NodeMergeService {
  ...
  async proposeMerge(request: IMergeRequest): Promise<IMergedContentProposal> {
    ...
    if (strategy === 'ai') {
      ...
      if (!apiKey) {
          throw new Error("AI API Key not configured. Please enable AI in settings or use Manual Merge.");
      }
    ...
    const outgoingConnections = sources.flatMap(s => s.connections.map(c => ({
      ...c,
   ...
   */
  async executeMerge(finalContent: IMergedContentProposal, sourceIds: string[]): Promise<void> {
    const { targetId, suggestedFrontmatter, suggestedBody, outgoingConnections } = finalContent;
    ...
    const newConnections = outgoingConnections.filter(newC => 
      !existingConnections.some(exC => exC.target === newC.target)
    );
    ...
    const finalConnections = [
      ...existingConnections, 
      ...newConnections.map(c => ({ target: c.target, label: c.label, strength: 1, type: 'default' })) // Map back to schema Connection
    ...
    for (const entity of allEntities) {
        ...
        for (const sourceTitle of sourceTitles) {
            ...
            if (newContent.match(regex)) {
                newContent = newContent.replace(regex, `[[${targetTitle}$1]]`);
                contentModified = true;
            }
            if (newLore.match(regex)) {
                newLore = newLore.replace(regex, `[[${targetTitle}$1]]`);
                contentModified = true;
            }
        }
        ...
        for (const sourceId of sourceIds) {
             const connIndex = newConnections.findIndex(c => c.target === sourceId);
             if (connIndex !== -1) {
                 const oldConn = newConnections[connIndex];
                 // Check if connection to target already exists to avoid duplicates
                 const exists = newConnections.some(c => c.target === targetId);
                 if (!exists) {
                     newConnections[connIndex] = { ...oldConn, target: targetId };
                 } else {
