...
describe("AIService Context Retrieval", () => {
  ...
  it("should prioritize explicit title matches over active selection", async () => {
    ...
    const { primaryEntityId } = await aiService.retrieveContext(
  ...
  it("should match short titles only with word boundaries", async () => {
    ...
    const { primaryEntityId: match1 } = await aiService.retrieveContext(
      "Tell me about AI",
      new Set(),
      vault,
      undefined,
    ...
    const { primaryEntityId: match2 } = await aiService.retrieveContext(
      "Training is hard",
      new Set(),
      vault,
      undefined,
  ...
  it("should include connection context in the retrieved text", async () => {
    ...
    // Outbound case
    const { content: contentOut } = await aiService.retrieveContext(
      "The Woods",
      new Set(),
      vault,
      undefined,
    ...
    // Inbound case
    const { content: contentIn } = await aiService.retrieveContext(
      "The Crone",
      new Set(),
      vault,
      undefined,
  ...
  it("should handle missing entities in connection context gracefully", async () => {
    ...
    const { content } = await aiService.retrieveContext(
      "The Woods",
      new Set(),
      vault,
      undefined,
  ...
  describe("Context Fusion & Prioritization", () => {
    ...
    it("should respect 10k character limit and prioritize selected entity", async () => {
      ...
      const { content, sourceIds } = await aiService.retrieveContext(
        "query",
        new Set(),
        vault,
        undefined,
    ...
    it("should populate sourceIds for all consulted entities", async () => {
      ...
      const { sourceIds } = await aiService.retrieveContext(
        "The Woods and Crone",
        new Set(),
        vault,
        undefined,
...
describe("AIService Prompt Enhancement", () => {
  ...
  it("should include context in the prompt when provided", () => {
    ...
    expect(result).toContain(
      "User visualization request: Show me what he looks like",
    );
  });
});
...
describe("AIService Style Caching", () => {
  ...
  it("should cache the style context after first lookup", async () => {
    ...
    expect(vi.mocked(searchService.search).mock.calls.length).toBe(
      callsAfterFirst + 1,
    ); // +1 for the main context search
  });
