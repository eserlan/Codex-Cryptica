...
import { getPersistedHandle, clearPersistedHandle, getDB, type VaultRecord } from "../utils/idb";
...
class VaultStore {
	...
	private async getActiveVaultHandle(): Promise<FileSystemDirectoryHandle | undefined> {
	...
	async listVaults(): Promise<VaultRecord[]> {
		...
		this.availableVaults = vaults.sort((a, b) => b.lastOpenedAt - a.lastOpenedAt);
	...
	async deleteVault(id: string): Promise<void> {
		if (id === this.activeVaultId) throw new Error("Cannot delete active vault");
	...
	async switchVault(id: string): Promise<void> {
		...
		// Ensure all pending changes are flushed before wiping memory
		await this.saveQueue.waitForAll();
		this.entities = {}; // Wipe early to prevent further enqueues from this vault
	...
	private async migrateStructure() {
		...
		try {
			...
			if (hasImages) {
				...
				const targetImagesDir = await defaultVaultDir.getDirectoryHandle('images', { create: true });
	...
	async syncToLocal() {
		...
		try {
			...
			this.status = "saving";
			debugStore.log(`Syncing Vault (${this.activeVaultId}) to local folder: ${localHandle.name}`);
			...
			const processFile = async (fileEntry: FileEntry) => {
				...
				if (cached && cached.lastModified === lastModified) {
					...
					const id =
						metadata.id ||
						sanitizeId(
							fileEntry.path[fileEntry.path.length - 1].replace(/\.(md|markdown)$/i, ""),
						);
	...
	async resolveImageUrl(path: string): Promise<string> {
		...
		if (/^https?:\/\//.test(path)) {
			try {
				const cacheDir = await vaultDir.getDirectoryHandle(".cache", { create: true });
				const externalDir = await cacheDir.getDirectoryHandle("external_images", { create: true });

				// Create a unique filename based on the URL
				const safeName = path.replace(/[^a-z0-9]/gi, '_').toLowerCase().slice(-100) + ".cache";
				...
				try {
					...
					try {
						...
						// CORS failure or other error, try proxy fallback
						console.log(`Direct fetch failed for ${path}, trying proxy...`, err);
						const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(path)}`;
						const proxyResponse = await fetch(proxyUrl);
						if (!proxyResponse.ok) throw new Error("Proxy fetch failed");
						const data = await proxyResponse.json();

						// allorigins returns the content as a data URI in 'contents'
						const contentResponse = await fetch(data.contents);
						blob = await contentResponse.blob();
					}
		...
		try {
			...
			for (const { sourceId } of inbound) {
				...
				if (sourceEntity) {
					...
					const connsToRemove = sourceEntity.connections.filter(c => c.target === id);
