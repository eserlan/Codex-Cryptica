...
describe("AdventureManager", () => {
  it("uses the injected ID generator for sessions, turns, and local records", async () => {
    const ids = [
      "session-id",
      "opening-turn-id",
      "opening-input-id",
    ...
    const idGenerator = vi.fn(() => ids.shift() ?? "unexpected-id");
    ...
    expect(manager.session?.id).toBe("session-id");
    expect(
      manager.session?.turns.map((turn) => [turn.id, turn.inputId]),
    ).toEqual([
      ["opening-turn-id", "opening-input-id"],
      ["action-turn-id", "action-input-id"],
    ]);
    expect(manager.session?.dicePresets[0]?.id).toBe("preset-id");
    expect(manager.session?.resourceCounters[0]?.id).toBe("counter-id");
    expect(idGenerator).toHaveBeenCalledTimes(7);
  });

  it("does not persist a session when ID generation fails", async () => {
    const deps: any = dependencies();
    const save = vi.spyOn(deps.repository, "save");
    const idGenerator = vi.fn(() => {
      throw new Error("id-generation-failed");
    });
    const manager = new AdventureManager({ ...deps, idGenerator });
