...
A **Markdown Code** tab sits alongside it for anyone who wants to write the underlying source directly: field references like `{{stat.hp display="current-max"}}` plus layout blocks (`:::stat-group`, `:::section`, `:::card`, `:::row`). The two tabs stay in sync: edit visually and the source updates underneath; edit the source and it reflects back into cards. It doesn't run HTML, CSS, JavaScript, or expressions, so imported layouts stay predictable.
...
Presentation Templates don't currently have a community directory of their own, so sharing one means exporting the file and handing it to whoever needs it, the way you would a Stat Template package before this directory existed.
