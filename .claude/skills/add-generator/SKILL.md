...
3. In `GeneratorPageContent.svelte`:
   ...
   - **If `<Name>FormFields.svelte`'s "Surprise Me" handler randomizes `genre` itself** (not every generator's does — World's deliberately leaves genre alone so its Surprise Me only varies the other fields; a generator with its own genre selector, like star-system, does randomize it), the handler MUST call `onGenreChange?.(genre)` with the freshly-rolled value before/alongside `onSurprise?.()`, exactly like the genre `<SelectWithCustomOption>`'s own `onvaluechange` does. Skipping this leaves `activeTheme` (and the page's visual skin) stuck on the pre-reroll genre after a Surprise Me click, even though the generated draft used the new one — a real, easy-to-miss gap when copy-pasting an existing FormFields component whose Surprise Me doesn't touch genre. Write a component test asserting `onGenreChange` fires on Surprise Me click, not just on manual select changes.
