...
- [ ] T002 Implement a retry mechanism or `await vaultRegistry.init()` check in `WorkerBridge.startSync` to ensure the registry is ready before reading vault metadata.
...
- [ ] T004 Manual verification: Toggle sync on a vault, ensure no console warnings about "active vault does not have GDrive sync enabled".
