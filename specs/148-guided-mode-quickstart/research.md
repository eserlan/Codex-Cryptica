...
- *Inline component generation in SvelteKit page*: Rejected—violates Principle I (Library-First) and duplicates generator code.
- *Pure LLM prompt without local tables*: Rejected—violates offline capability requirement (`FR-005`) and increases startup latency.
...
### Alternatives Considered
- *IndexedDB per-world field*: Rejected—clarification decision established that Guided Mode is a global user preference across all worlds, not isolated per world.
...
### Alternatives Considered
- *Redirecting to standard full generator page*: Rejected—creates modal jump and cognitive overload for new users (`FR-009`).
...
### Alternatives Considered
- *LLM-driven recommendation requests*: Rejected—adds unnecessary API latency and costs for simple structural checks.
