...
3. **Given** a Stat Sheet schema, **When** the user browses available presentations for it, **Then** only templates declared compatible with that schema (or with explicit compatible field mappings) are offered.
...
**Independent Test**: Take a valid template, rename/remove a referenced schema field, and confirm the sheet still renders (via clear in-place messaging for the missing field, not a crash); separately, attempt to import a template file containing raw HTML/script content and confirm it is rejected or stripped rather than rendered as-is; separately, select an invalid/corrupted template on an entity and confirm the entity falls back to the standard renderer.
...
2. **Given** a template file containing disallowed content (raw HTML tags, script blocks, executable expressions), **When** the user imports it, **Then** the disallowed content is rejected or safely stripped, and the user is informed which parts were not imported.
...
- What happens when two presentation templates for the same schema have the same name? Names must be disambiguated (e.g. per-vault uniqueness or automatic suffixing); this must not overwrite an existing template silently.
...
- **FR-004**: System MUST reject or safely strip arbitrary HTML tags, CSS, JavaScript, executable expressions, and any other user-authored script content from template source, on both authoring/save and import.
...
- **FR-007**: System MUST let users select which saved presentation template an entity's Stat Sheet uses, independent of and without altering the entity's stored stat values.
...
- **FR-017**: System MUST let users delete a vault-owned (non-built-in) presentation template, and any entity currently using it at deletion time MUST fall back per FR-010 rather than error.
...
- **FR-020**: System MUST allow a presentation template to declare the schema or field identifiers it expects, and MUST allow reuse of a template across more than one schema only where the field mapping between template and schema is explicit and compatible.
...
- **Presentation Template**: A named, versioned unit of extended-Markdown source plus parsed/validated layout and field-reference metadata. Declares the schema(s)/fields it expects. Is either built-in (read-only, duplicable) or vault-owned (editable, deletable). Value-free — contains no entity data.
- **Entity Stat Sheet Selection**: The association between a given entity's Stat Sheet (schema + data) and the presentation template currently selected for it. Independent of both the schema and the template's other consumers.
...
- **SC-003**: 100% of import attempts containing raw HTML, script tags, or executable expressions are rejected or stripped — none render as live HTML/script in the app.
...
- "Compatible schema" for template selection means the template's declared field identifiers exist on the schema and pass FR-005 validation; exact matching rules (strict vs. partial) are left to the implementation as long as incompatibility is always surfaced, not hidden.
