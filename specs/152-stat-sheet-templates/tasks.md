...
- [ ] T004 [P] Define `PresentationAst` node types (`Heading`, `Paragraph`, `List`, `Table`, `Blockquote`, `ThematicBreak`, `Image`, `Section`, `Group`, `Card`, `Row`, `FieldReference`, `UnknownDirective`, `MissingField`) in `packages/stat-sheet-engine/src/presentation/ast.ts` per data-model.md
...
- [ ] T023 [US2] Implement `GroupNode.svelte`, `SectionNode.svelte`, `CardNode.svelte`, `RowNode.svelte` (responsive containers, degrade below requested `columns` per FR-018) in `apps/web/src/lib/components/stats/presentation/nodes/` (depends on T011)
- [ ] T024 [US2] Implement `PresentationTemplateEditor.svelte` (Markdown source textarea + live preview via `<PresentationRenderer>` in `preview` mode + inline diagnostics for `MissingField`/`UnknownDirective`) in `apps/web/src/lib/components/stats/presentation/PresentationTemplateEditor.svelte`, making T022 pass (depends on T009, T023)
- [ ] T025 [US2] Implement field-reference autocomplete popover in the editor, insertable at cursor, making T021 pass (depends on T021, T024)
- [ ] T026 [US2] Implement create-from-scratch, duplicate-built-in, and duplicate-existing flows with per-schema name uniqueness (auto-suffix or block on collision, per data-model.md Validation Rules) in `PresentationTemplateEditor.svelte`, persisting via `PresentationTemplateStore` (depends on T010, T016, T024)
- [ ] T027 [US2] Add a "Presentation Templates" section to the Stat Sheet template management area (entry point alongside existing `StatSheetTemplateModal.svelte`) linking to the editor and picker (depends on T018, T026)
...
- [ ] T028 [P] [US3] Write failing unit tests in `packages/stat-sheet-engine/src/presentation/presentation.test.ts`: renaming/removing a referenced schema field turns that `FieldReference` into `MissingField` without invalidating the rest of the template (`isTemplateUsable` stays true)
...
- [ ] T030 [P] [US3] Write failing component test in `apps/web/src/lib/components/stats/DetailStatsTab.test.ts`: an entity whose `presentationTemplateId` points at a missing/deleted template renders via `StatSheetView.svelte` (FR-010), not an error state

### Implementation for User Story 3

- [ ] T031 [US3] Implement `MissingFieldNode.svelte` and `UnknownDirectiveNode.svelte` (visible flagged placeholders, plain-language messaging per Constitution IX) in `apps/web/src/lib/components/stats/presentation/nodes/`, making T028 pass in the rendered UI (depends on T009)
- [ ] T032 [US3] Implement the strip-and-report sanitization pass (reject raw HTML/CSS/JS/executable expressions, keep the rest) as part of `parseTemplate()`/`validateAst()` used by both save and import paths in `packages/stat-sheet-engine/src/presentation/parse.ts` and `packages/stat-sheet-engine/src/presentation/package.ts`, making T029 pass (depends on T007, T009)
- [ ] T033 [US3] Implement deletion-fallback rules in `packages/stat-sheet-engine/src/presentation/resolve.ts` and `PresentationTemplateStore`: deleting a schema's default resets it to `null`; deleting a template referenced by an entity override leaves a dangling id that `resolvePresentationTemplate` treats as invalid → fallback (per data-model.md Validation Rules), making T030 pass (depends on T015, T033 wiring into T012's fallback check)
- [ ] T034 [US3] Manually run quickstart.md §3 and confirm all four scenarios pass end-to-end
...
- [ ] T036 [P] [US4] Write failing unit tests: importing a package whose `schemaTemplateId` has no match among the destination vault's schemas returns a typed incompatibility result rather than silently attaching to an unrelated schema

### Implementation for User Story 4

- [ ] T037 [US4] Implement `exportTemplate()`/`importTemplatePackage()` in `packages/stat-sheet-engine/src/presentation/package.ts`, modeled on `template-package.ts`'s existing versioned-envelope pattern, making T035 and T036 pass (depends on T032)
- [ ] T038 [US4] Add export/import buttons and incompatibility messaging to the Presentation Templates management area in `apps/web/src/lib/components/stats/presentation/PresentationTemplateEditor.svelte` (depends on T027, T037)
- [ ] T039 [US4] Manually run quickstart.md §4 and confirm all three scenarios pass end-to-end
...
- [ ] T040 [P] Add a Presentation Templates entry to `apps/web/src/lib/config/help-content.ts` and a `FeatureHint` for first-time use of the editor (Constitution VII)
- [ ] T041 [P] Pass over all diagnostic/flag copy (`MissingField`, `UnknownDirective`, import-stripped notices, incompatibility messages) for plain, non-technical language (Constitution IX)
- [ ] T042 Run `bun run lint` and `bun run test` across `packages/stat-sheet-engine` and `apps/web`, confirming the new code meets the 70% coverage goal (Constitution X)
- [ ] T043 Run `.specify/scripts/bash/check-prerequisites.sh` equivalent full manual pass of quickstart.md end-to-end (all four stories in one continuous session)
- [ ] T044 Update `AGENTS.md` Active Technologies note if any implementation detail diverged from plan.md
