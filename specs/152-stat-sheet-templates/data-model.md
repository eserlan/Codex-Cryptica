...
Extends the existing `StatSheetTemplateSchema` record (`packages/schema/src/stat-sheet.ts`) with one new optional field:

| Field | Type | Notes |
|---|---|---|
| `defaultPresentationTemplateId` | string \| null | `PresentationTemplate.id`; `null` = use the standard (non-templated) renderer as the schema's default |
