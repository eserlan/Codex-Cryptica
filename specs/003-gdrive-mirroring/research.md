...
- **Scopes**: `https://www.googleapis.com/auth/drive.file` (Recommended). This grants access *only* to files created or opened by the app, ensuring privacy and reducing security risk. The constitution requires "Total Data Sovereignty", so accessing only *our* data is perfect.
...
5.  **Deletions**:
    ...
    - If file exists in IDB but missing in Remote -> Delete Local (or Re-upload? *Decision: Re-upload to be safe, treat as "Local New"*).
...
- `gapi` might be tricky in a Worker (relies on global `window`).
    - *Fallback*: If `gapi` fails in Worker, we will use raw `fetch` endpoints for Drive API, passing the token from the main thread to the worker. This is actually cleaner and smaller bundle-wise.
