...
5. **Collapsed indicators**: one entry per collapsed branch root, with `hiddenCount` taken from the lineage's `siblingBranches` entry (computed by `buildLineage` even while collapsed — see buildLineage guarantee 5; FR-008's "roughly how much" indicator).
...
- `PanZoomContainer.svelte`: emits nothing outside; owns `viewport` via `pan-zoom.svelte.ts`; sets `touch-action: none` and clips content (page never overflows, FR-014); exposes `fitTo(bounds)` for initial framing. The container is keyboard-operable (FR-007): focusable, arrow keys pan, `+`/`-` zoom — reusing the map's `getKeyboardViewportUpdate` where it fits.
