...
- **Decision**: `buildLineage` takes `{ maxUp?: number; maxDown?: number }`. Default initial render uses **N = 3** generations each way (tunable constant in the view, not the engine); beyond the cap the result marks boundary members `hasMoreUp`/`hasMoreDown` so the view draws "⌃ M more generations" expanders. Expanders raise the cap stepwise (+3); "Expand all" re-runs with no cap **and** marks all sibling branches expanded. Collapse state lives in a `Set<entityId>` of expanded branch roots + current caps, all transient component state.
