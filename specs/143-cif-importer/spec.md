...
- **FR-015**: On re-import, records whose stable identity matches an existing vault entity MUST be offered as update or skip (showing which fields an update would change); unmatched records are offered as create. Accepting every "skip" on an identical re-import MUST leave the vault unchanged.
- **FR-016**: An update MUST be additive and corrective only: it may change the imported entity's fields and add new links, but MUST NOT remove relationships or content the user created in the app since the previous import.
