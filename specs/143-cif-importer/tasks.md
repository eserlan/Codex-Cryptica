...
- [ ] T009 [US1] Wire CIF into `apps/web/src/lib/components/settings/import-settings-controller.svelte.ts`: detect CIF before the chronica/scabard checks (filename `.cif.json`, or parsed `format === "codex-world-interchange"`), run `parseCifFile` → `normalizeCifPackage` → existing engine `prepare()` with the CIF `sourceRefBuilder`, surface world title/description in the review session header; controller tests for detection order, the happy path, cancel-at-review discarding the session with zero vault mutations (FR-009), and a guest-session assertion that the CIF path is unreachable (FR-019)
