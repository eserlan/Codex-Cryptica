...
- **FR-035**: A conversation MUST have a maximum number of completed turns (30) as a cost safeguard. The limit MUST NOT be shown or hinted at while there is room (no "turns left", no "turn N of M", no number in help or privacy text). Only if a user reaches it, the tool MUST say so in plain language and offer a new conversation or copying the result.
...
- Starting values, adjustable during build: maximum idea or turn text length 4,000 characters; 30 completed turns per conversation, not shown to the user; per-browser limit of a 10-second cooldown and 60 turns per hour, enough for two whole conversations. The length limit is shown to the user next to the input.
