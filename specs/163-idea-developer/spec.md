...
- **FR-038**: Before the user's first submit in a conversation, and for as long as the conversation is active, the tool MUST show a plain-language notice next to the submit action (not only in help or a policy page) saying: the idea and later turns are sent to the AI provider, the provider keeps the conversation while it continues so earlier turns do not have to be resent, Codex Cryptica does not keep it, and how to end the conversation. The notice MUST NOT block use behind a checkbox or dialog, MUST NOT be hidden behind a collapsed section, and MUST link to the fuller explanation in the privacy page and the help entry.
...
- **SC-021**: In a review of the tool with first-time users or a walkthrough script, 100% of first submits are preceded by a visible notice next to the submit action, and the notice, help entry and privacy page agree on what is kept, by whom, and how to end the conversation.
