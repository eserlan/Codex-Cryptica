...
- _Lazy Cleanup_: Only remove the link when the referring node is next saved. (Rejected: Leads to inconsistent graph state in the short term).
- _Virtual Cleanup_: Only hide the link in the UI. (Rejected: Violates "Sovereign Data" - the files should be accurate).
