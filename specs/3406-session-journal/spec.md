...
**Feature Branch**: `3406-session-journal`
**Created**: 2026-09-25
**Status**: Draft
...
- **FR-011**: System MUST persist journal data (journals, sections, entries) locally so that it survives an app reload, and MUST NOT silently lose entries when the same vault is open in more than one browser tab (last-write-wins at the entry level is acceptable; losing an entire journal is not).
