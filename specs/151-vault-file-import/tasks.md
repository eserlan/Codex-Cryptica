...
**Independent Test**: Select a mix of files where some exactly path-match existing vault entities (previously imported via this same flow) and some don't, including one that merely shares a _title_ (but not path) with an unrelated existing entity; confirm; verify only the true path-matches were skipped, the title-only-coincidence file was still created, and every existing target entity's fields are byte-for-byte unchanged.
