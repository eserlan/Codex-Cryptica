...
- Produces one `EntityDraft` with `sourcePath` set to the file's `relativePath`, and `sourceType` set to the file's real (free-form, per-vault-defined) frontmatter `type`. `buildVaultFilesMappingRules` (`packages/importer/src/vault-files/mapping.ts`) then derives one passthrough rule per distinct `sourceType` seen in the batch, so each entity resolves back to its own real type instead of falling through to `DEFAULT_MAPPING_RULES`'s `"note"` default — a static rule table isn't used, since `EntityTypeSchema` is `z.string()` (Flexible Categories), not a fixed enum. `image`/`thumbnail` are populated from the entity's frontmatter references.
