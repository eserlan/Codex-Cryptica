...
- `docx`: Mostly for _creating_ files, reading is limited.
...
_Constraint_: Image extraction from PDF is notoriously difficult (masked images, CMYK, vector parts).
_MVP Adjustment_: For MVP, we might prioritize _Text_ from PDFs and _Text+Images_ from DOCX, unless PDF image extraction proves straightforward with `pdfjs`.
