...
- [ ] T011 [P] [US1] Unit test for token placement logic in `apps/web/tests/unit/stores/map-session.test.ts` (extend existing)
- [ ] T012 [P] [US1] Unit test for grid snapping math in `apps/web/tests/unit/lib/vtt-helpers.test.ts`
- [ ] T013 [US1] Integration test: token drag + render cycle in `apps/web/tests/e2e/vtt-token-drag.spec.ts`

### Implementation for User Story 1

- [ ] T014 [US1] Add token add/remove methods to MapSession store in `apps/web/src/lib/stores/map-session.svelte.ts`
- [ ] T015 [US1] Implement grid snapping helper in `apps/web/src/lib/utils/vtt-helpers.ts` (snap to `mapStore.gridSize`)
- [ ] T016 [US1] Create VTTControls.svelte with VTT mode toggle in `apps/web/src/lib/components/map/VTTControls.svelte`
- [ ] T017 [US1] Wire VTT mode toggle to MapView.svelte draw() loop to enable token overlay rendering
- [ ] T018 [US1] Add token placement UI (click to place + dialog for name/entity link) in `apps/web/src/lib/components/map/TokenAddDialog.svelte`
- [ ] T019 [US1] Implement pointer-based token drag in MapView.svelte (pointer down/move/up with coordinate transform)
- [ ] T020 [US1] Add grid snapping to token drag using vtt-helpers.ts
- [ ] T021 [US1] Render token labels as canvas text in `renderTokens()` for freeform tokens
- [ ] T022 [US1] Render token images from linked entities in `renderTokens()` using entity image blob URLs
- [ ] T023 [US1] Clamp token positions to map bounds to prevent off-map placement
- [ ] T024 [US1] Handle deleted vault entity gracefully (token displays cached name as freeform)
- [ ] T025 [US1] Persist session tokens within the tab (survive page refresh within the same session)
...
- [ ] T026 [P] [US2] Unit test for selection state management in `apps/web/tests/unit/stores/map-session.test.ts` (extend existing)
- [ ] T027 [US2] Unit test for token hit-testing math in `apps/web/tests/unit/lib/vtt-helpers.test.ts` (extend existing)

### Implementation for User Story 2

- [ ] T028 [US2] Add token hit-testing in MapView.svelte pointer handler (detect click on token bounds)
- [ ] T029 [US2] Wire token click to MapSession store selection state
- [ ] T030 [US2] Create TokenDetail.svelte side panel in `apps/web/src/lib/components/vtt/TokenDetail.svelte`
- [ ] T031 [US2] Render token selection highlight (glow/border) in `renderTokens()` canvas function
- [ ] T032 [US2] Implement deselect on empty-space click in MapView.svelte
...
- [ ] T033 [P] [US3] Unit test for initiative cycling logic in `apps/web/tests/unit/stores/map-session.test.ts` (extend existing)
- [ ] T034 [US3] Unit test for turn advancement and round counting in `apps/web/tests/unit/stores/map-session.test.ts` (extend existing)

### Implementation for User Story 3

- [ ] T035 [US3] Add initiative management methods to MapSession store (addToken, removeToken, reorder, advanceTurn)
- [ ] T036 [US3] Create InitiativePanel.svelte in `apps/web/src/lib/components/vtt/InitiativePanel.svelte`
- [ ] T037 [US3] Add drag-reorder capability to initiative list entries in InitiativePanel.svelte
- [ ] T038 [US3] Render active token highlight in `renderTokens()` when session is in combat mode
- [ ] T039 [US3] Add round counter display in InitiativePanel.svelte
- [ ] T040 [US3] Implement mode toggle (exploration ↔ combat) in MapSession store and VTTControls.svelte
- [ ] T041 [US3] Lock token movement to active token when in combat mode (non-active tokens cannot be moved)
...
- [ ] T042 [P] [US4] Unit test for distance calculation math in `apps/web/tests/unit/lib/vtt-helpers.test.ts` (extend existing)

### Implementation for User Story 4

- [ ] T042 [US4] Add measurement tool state to MapSession store (startPoint, endPoint, active)
- [ ] T043 [US4] Create MeasurementTool.svelte overlay in `apps/web/src/lib/components/map/MeasurementTool.svelte`
- [ ] T044 [US4] Implement measurement click handler in MapView.svelte (pointer events on measurement tool active)
- [ ] T045 [US4] Draw measurement line and distance label on canvas in `renderTokens()` or dedicated `renderMeasurement()` function
- [ ] T046 [US4] Display distance using map scale if defined, otherwise pixel distance
- [ ] T047 [US4] Add measurement tool toggle to VTTControls.svelte
...
- [ ] T048 [P] [US5] Unit test for host token move broadcast in `apps/web/tests/unit/services/vtt-session.test.ts` (extend existing)
- [ ] T049 [P] [US5] Unit test for guest permission rejection in `apps/web/tests/unit/services/vtt-session.test.ts` (extend existing)
- [ ] T050 [US5] E2E test: host joins, guest joins, host moves token, guest sees update in `apps/web/tests/e2e/vtt-session.spec.ts`

### Implementation for User Story 5

- [ ] T051 [US5] Implement SESSION_SNAPSHOT broadcast on guest join in host-service.svelte.ts
- [ ] T052 [US5] Implement TOKEN_MOVE handler in host-service.svelte.ts (validate ownership, update, broadcast TOKEN_STATE_UPDATE)
- [ ] T053 [US5] Implement TOKEN_MOVE sender in guest-service.ts (send to host, handle timeout revert)
- [ ] T054 [US5] Implement TOKEN_ADD_REQUEST handler in host-service.svelte.ts (host assigns id, ownerPeerId, broadcasts TOKEN_ADDED)
- [ ] T055 [US5] Implement TURN_ADVANCE broadcast from MapSession store through host-service when host advances turn
- [ ] T056 [US5] Add token ownership assignment UI in TokenDetail.svelte (GM assigns tokens to connected guests)
- [ ] T057 [US5] Implement guest permission enforcement in MapView.svelte drag handler (check ownerPeerId before allowing drag)
- [ ] T058 [US5] Implement session fog reveal sync (host paints fog → broadcasts FOG_REVEAL → guests apply session mask delta)
- [ ] T059 [US5] Implement MAP_PING broadcast for guest/host cursor pings
- [ ] T060 [US5] Handle host disconnection (detect peer disconnect, end session for all guests, clear session state)
...
- [ ] T061 [P] Write unit tests for encounter save/load serialization in `apps/web/tests/unit/stores/map-session.test.ts` (extend existing)
- [ ] T062 Create encounter snapshot save/load methods in MapSession store (serialize to OPFS JSON)
- [ ] T063 Create EncounterManager.svelte in `apps/web/src/lib/components/vtt/EncounterManager.svelte` (save/load UI)
- [ ] T064 Implement OPFS storage for encounter snapshots at `maps/{mapId}_encounter_{encounterId}.json`
- [ ] T065 Add encounter list display in EncounterManager.svelte (load previously saved encounters)
- [ ] T066 Implement session fog mask serialization (compress PNG blob → store in encounter JSON)
...
- [ ] T067 [P] Update user documentation with VTT feature help entries in `apps/web/src/lib/config/help-content.ts`
- [ ] T068 [P] Add FeatureHint for first-time VTT mode entry
- [ ] T069 Run full test suite and verify coverage meets 70% goal for new VTT code
- [ ] T070 Performance: verify 60fps token rendering with 20 tokens, optimize if needed
- [ ] T071 [P] E2E test: full combat round flow (place tokens → initiative → advance turns → end) in `apps/web/tests/e2e/vtt-combat-round.spec.ts`
- [ ] T072 Quickstart.md validation: follow the quickstart guide and verify all steps work
- [ ] T073 Code cleanup: remove any TODO comments, unused imports, debug logs
